...
import {first, get, find, findIndex, isEmpty, isNull, values,
  invert, clone, each, throttle, debounce, map, remove, trimEnd} from 'lodash';
...
export default class EditorArea extends React.Component {
  ...
  splitPage = () => {
    ...
    if (pageIndex < (pages.length - 1)) {
      ...
      const fromPageContent = trimEnd(firstPart) + secondPart;
  ...
  handleAppUndoPb = () => {
    ...
    if (record) {
      const doc = this.getDoc();
      const {writePageContent, setPageIndex} = this.props;
      const {docId, fromPageId, toPageId, fromPageContent, toPageContent} = record;
