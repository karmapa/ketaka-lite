...
export default class EditorArea extends React.Component {
  ...
  splitPage = () => {
    ...
    if (pageIndex < (pages.length - 1)) {
      ...
      // remove \n generated by splitPage
      const fromPageContent = content.substring(0, firstPart.length - 1) + content.substring(firstPart.length);

      this.lastPageBreakRecord = {
  ...
  handleAppUndoPb = () => {
    if (this.lastPageBreakRecord) {
      const doc = this.getDoc();
      const {writePageContent, setPageIndex} = this.props;
      const {fromPageId, toPageId, fromPageContent, toPageContent} = this.lastPageBreakRecord;
      ...
      setPageIndex(doc.uuid, fromPageIndex);
      this.lastPageBreakRecord = null;
    }
  };
